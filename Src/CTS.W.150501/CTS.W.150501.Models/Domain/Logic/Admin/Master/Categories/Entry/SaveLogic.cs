    ...
    public class SaveLogic
    {
        ...
        private void Check(SaveDataModel inputObject)
        {
            ...
            if (DataCheckHelper.IsNull(dataInfo.LocaleCd)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_CATEGORIES_00002"));
            }
            if (DataCheckHelper.IsNull(dataInfo.CategoryCd)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_CATEGORIES_00003"));
            }
            if (DataCheckHelper.IsNull(dataInfo.CategoryName)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_CATEGORIES_00004"));
            }
            if (DataCheckHelper.IsNull(dataInfo.LinkName)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_CATEGORIES_00006"));
            }
            ...
            var isExist = masterDataCom.IsExistCategory(dataInfo.LocaleCd, dataInfo.CategoryCd, true);
            var isExistSEO = seoCom.IsExist(dataInfo.LocaleCd, W150501Logics.GRPSEO_MA_CATEGORIES, dataInfo.CategoryCd, true);
            // Kiểm tra dữ liệu tồn tại trường hợp status là add
            if (inputObject.IsAdd && (isExist || isExistSEO)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00017", "ADM_MA_CATEGORIES_00001"));
            }
            ...
            var isUnique = masterDataCom.IsUniqueCategory(dataInfo.CategoryCd, dataInfo.LinkName);
            // Kiểm tra giá trị duy nhất
            if (!isUnique) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00017", "ADM_MA_CATEGORIES_00006"));
            }
            // Kiểm tra danh sách lỗi
            if (!DataCheckHelper.IsNull(msgs)) {
                throw new ExecuteException(msgs);
            ...
            foreach (var info in listLocale) {
                ...
                if (DataCheckHelper.IsNull(info.LocaleCd)) {
                    ...
                    msgs.Add(MessageHelper.GetMessageForList(
                        "ADM_MA_CATEGORIES_00011", i, "E_MSG_00001", "ADM_MA_CATEGORIES_00002"));
                }
                if (DataCheckHelper.IsNull(info.CategoryName)) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "ADM_MA_CATEGORIES_00011", i, "E_MSG_00001", "ADM_MA_CATEGORIES_00004"));
                }
                ...
                if (dataInfo.LocaleCd == info.LocaleCd || listLocale.Contains(info, comparer)) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "ADM_MA_CATEGORIES_00011", i, "E_MSG_00017", "ADM_MA_CATEGORIES_00002"));
                }
                // Trường hợp lỗi thì đi đến record tiếp theo
                if (flagError) {
                    // Tăng giá trị i
        ...
        private SaveDataModel SaveInfo(SaveDataModel inputObject)
        {
            ...
            var listLocaleDb = processDao.GetListLocale(dataInfo.CategoryCd);
            // Khởi tạo comparer
            var comparer = new KeyEqualityComparer<CategoryObject>((k1, k2) =>
